...
def get_neighbors_indices(training_inputs, test_instance, k):
    ...
    """
    # TODO
    dists = np.array([euclidean_distance(train_input, test_instance) for train_input in training_inputs])
    sorted_dists_index = dists.argsort()
    return sorted_dists_index[:k]
...
def get_response(training_labels, neighbor_indices):
    ...
    """
    # TODO
    votes = [training_labels[index] for index in neighbor_indices]
