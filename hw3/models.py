...
from kneighbors import get_neighbors_indices, get_response, euclidean_distance
from kmeans import kmeans
from dtree import *
...
class KNeighborsClassifier(object):
    ...
    def predict(self, X):
        ...
        for i in range(len(X)):
            indices = get_neighbors_indices(self.train_inputs, X[i], self.k)
            res.append(get_response(self.train_labels, indices))
    ...
    def accuracy(self, data):
        ...
        for i in range(len(data.labels)):
            if data.labels[i] == predictions[i]:
                cnt += 1

        return cnt / len(data.labels)
...
class KmeansClassifier(object):
    ...
    def train(self, X, y):
        ...
        for label in np.unique(y):
            X_now = X[y == label]
            self.cluster_centers_[label] = kmeans(X_now, self.k, self.max_iter, self.tol)
    ...
    def predict(self, X):
        ...
        for i in range(len(X)):
            min_dis = 10000
            min_label = None
            for k,v in self.cluster_centers_.items():
                for centroid in v:
                    if euclidean_distance(centroid, X[i]) < min_dis:
                        min_dis = euclidean_distance(centroid, X[i])
                        min_label = k
            res.append(min_label)
